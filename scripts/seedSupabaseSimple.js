...
async function seedDatabase() {
  try {
    console.log('🌱 Starting simplified Supabase database seeding...');
    ...
    // 4. Create simple sprints (without is_active field)
    ...
    for (let year = currentYear; year < currentYear + 2; year++) {
      for (let quarter = 1; quarter <= 4; quarter++) {
        ...
        sprints.push({
          ...
          year: year,
          quarter: quarter
          // Removed is_active field to avoid schema mismatch
        });
      }
    }
